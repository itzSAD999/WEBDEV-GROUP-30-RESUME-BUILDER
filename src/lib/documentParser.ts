...
function extractDateRange(text: string): { startDate: string; endDate: string; current: boolean } {
  const patterns = [
    ...
    new RegExp(`(${MONTH_NAMES.source}[\\s,]*\\d{4})\\s*[-–—to]+\\s*((?:expected\\s+)?${MONTH_NAMES.source}[\\s,]*\\d{4}|present|current|now|ongoing)`, 'i'),
...
function stripDateRange(text: string): string {
  return text
    .replace(new RegExp(`\\s*(${MONTH_NAMES.source}[\\s,]*\\d{4}|\\d{4})\\s*[-–—to]+\\s*((?:expected\\s+)?${MONTH_NAMES.source}[\\s,]*\\d{4}|\\d{4}|present|current|now|ongoing)\\s*`, 'gi'), '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}
