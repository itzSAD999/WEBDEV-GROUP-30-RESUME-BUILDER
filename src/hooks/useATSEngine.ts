...
export interface ResumeQualityScore {
  ...
  ats: { score: number; max: number; details: string[] };
  completeness: { score: number; max: number; details: string[] };
  impact: { score: number; max: number; details: string[] };
  ready: { score: number; max: number; details: string[] };
}

export interface ATSEngineState {
  resumeSource: "builder" | "uploaded";
...
export interface ATSEngineActions {
  setResumeSource: (source: "builder" | "uploaded") => void;
  setUploadedResumeText: (text: string | null) => void;
  setJobDescription: (jd: string) => void;
  calculateQualityScore: () => ResumeQualityScore;
  analyzeJobMatch: () => Promise<ATSAnalysisResult | null>;
  analyzeCVContent: () => Promise<CVAnalysisResult | null>;
  clearJobMatch: () => void;
}

const generateCacheKey = (resumeData: ResumeData, jd: string) => {
...
const getResumeHash = (data: ResumeData) => JSON.stringify({
...
export function useATSEngine(resumeData: ResumeData): [ATSEngineState, ATSEngineActions] {
  const [resumeSource, setResumeSource] = useState<"builder" | "uploaded">("builder");
  const [uploadedResumeText, setUploadedResumeText] = useState<string | null>(null);
  const [jobDescription, setJobDescription] = useState("");
  const [jobMatchResult, setJobMatchResult] = useState<ATSAnalysisResult | null>(null);
  const [cvAnalysisResult, setCvAnalysisResult] = useState<CVAnalysisResult | null>(null);
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [isAnalyzingCV, setIsAnalyzingCV] = useState(false);
  const [lastAnalyzedAt, setLastAnalyzedAt] = useState<Date | null>(null);
  const [error, setError] = useState<string | null>(null);
  const cacheRef = useRef<Map<string, ATSAnalysisResult>>(new Map());
  const lastCVAnalysisHash = useRef<string>("");
  const lastJobMatchHash = useRef<string>("");

  const calculateQualityScore = useCallback((): ResumeQualityScore => {
    const contentDetails: string[] = [];
    const atsDetails: string[] = [];
    const completenessDetails: string[] = [];
    const impactDetails: string[] = [];
    const readyDetails: string[] = [];
  ...
  const analyzeJobMatch = useCallback(async (): Promise<ATSAnalysisResult | null> => {
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke('ats-analyzer', {
        body: {
          resumeData,
          jobDescription,
          resumeText: resumeSource === "uploaded" ? uploadedResumeText : undefined
        }
      });
  ...
  const analyzeCVContent = useCallback(async (): Promise<CVAnalysisResult | null> => {
    setIsAnalyzingCV(true);
    setError(null);

    try {
      const { data, error: invokeError } = await supabase.functions.invoke('cv-analysis', {
        body: {
          resumeData,
          resumeText: resumeSource === "uploaded" ? uploadedResumeText : undefined
        }
  ...
  const actions: ATSEngineActions = {
